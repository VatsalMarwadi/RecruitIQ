...
export default function AptitudeResultManagement() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to confirm results");
    } finally {
      setConfirming(false);
    }
  };
  ...
  const getCurrentStatus = (candidateId) => {
    ...
    const existingResult = results.find(
      (r) => (r.attempt_id || r.submission_id) === candidateId,
    );
      ...
      {!showPreview ? (
        <div className="bg-white rounded-lg border border-gray-200 p-6 max-w-2xl">
          ...
          <p className="text-xs text-gray-500 mt-2">
            <span className="font-medium">Note:</span> Preview will show
            suggested pass or fail based on the percentage. You can manually
            override individual results before confirming.
          </p>
        </div>
      ) : (
        // Preview Results
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  <th className="px-6 py-2.5 text-left text-xs font-medium text-gray-500 uppercase">
                    Candidate
                  </th>
              ...
              <tbody className="divide-y divide-gray-50">
                {previewData?.results?.map((result) => {
                  const candidateId = result.attempt_id || result.submission_id;
                  const suggested = result.suggested_result;
                  const current = getCurrentStatus(candidateId);
                  const isModified =
                    modifiedResults[candidateId] &&
                    modifiedResults[candidateId] !== suggested;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-3 text-center">
                        <span
                          className={`font-medium ${(result.percentage || 0) >= passingPercentage ? "text-green-600" : "text-red-600"}`}
                        >
                          {result.percentage}%
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-3 text-center">
                        {!isEvaluated ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-50 text-yellow-700">
                            <FaClock className="w-3 h-3" /> Evaluation Pending
                          </span>
                        ) : (
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
          ...
          <div className="px-6 py-3 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500">
            <span>Showing {previewData?.results?.length || 0} candidates</span>
            <div className="flex items-center gap-4">
              <span className="flex items-center gap-1 text-green-600">
                <span className="text-green-600">●</span>
                {previewData?.results?.filter((r) => {
                  const id = r.attempt_id || r.submission_id;
                  return (
                    (modifiedResults[id] || r.suggested_result) === "passed"
                  );
                }).length || 0}{" "}
                Passed (Final)
              </span>
              <span className="flex items-center gap-1 text-red-600">
                ...
                {previewData?.results?.filter((r) => {
                  const id = r.attempt_id || r.submission_id;
                  return (
                    (modifiedResults[id] || r.suggested_result) === "failed"
                  );
                }).length || 0}{" "}
                Failed (Final)
              </span>
              <span className="flex items-center gap-1 text-yellow-600">
                ...
                {previewData?.results?.filter((r) => {
                  const id = r.attempt_id || r.submission_id;
                  return (
                    (modifiedResults[id] || r.suggested_result) === "pending"
                  );
                }).length || 0}{" "}
                Pending
              </span>

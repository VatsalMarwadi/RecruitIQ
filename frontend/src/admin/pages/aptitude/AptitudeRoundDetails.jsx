// admin/pages/aptitude/AptitudeRoundDetails.jsx
...
export default function AptitudeRoundDetails() {
  ...
  const fetchRoundDetails = async () => {
    try {
      ...
      if (roundResponse.data.success) {
        ...
        // Update stats from round details
        setStats(prev => ({
          ...prev,
      ...
      // Fetch results
      const resultResponse = await api.get(`/canadmin/list-aptitude-results/${roundId}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleViewQuestions = () => {
    navigate(`/admin/aptitude-round/${roundId}/questions`);
  };

  const handleManageResults = () => {
    navigate(`/admin/aptitude-round/${roundId}/manage-results`);
  };

  const handleViewCandidateResult = (attemptId) => {
    ...
    <div className="w-full">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {roundData?.round_type_display || 'Aptitude Round'}
          </h1>
          <div className="flex items-center gap-3 mt-0.5 flex-wrap">
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(roundData?.status)}`}>
              {roundData?.status ? roundData.status.charAt(0).toUpperCase() + roundData.status.slice(1) : 'Draft'}
            </span>
            <span className="text-sm text-gray-500">Round {roundData?.round_order || 1}</span>
            <span className="text-sm text-gray-400">•</span>
            <span className="text-sm text-gray-500">{roundData?.duration_minutes || 0} min</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <p className="text-xl font-semibold text-gray-900 mt-1">{stats.totalQuestions}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-xs text-gray-500 font-medium">Total Marks</p>
          <p className="text-xl font-semibold text-gray-900 mt-1">{stats.totalMarks}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-xs text-gray-500 font-medium">Candidates</p>
          <p className="text-xl font-semibold text-gray-900 mt-1">{stats.totalCandidates}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-xs text-gray-500 font-medium">Passed</p>
          <p className="text-xl font-semibold text-green-600 mt-1">{stats.passed}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-xs text-gray-500 font-medium">Failed</p>
          <p className="text-xl font-semibold text-red-600 mt-1">{stats.failed}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-xs text-gray-500 font-medium">Average Score</p>
          <p className="text-xl font-semibold text-gray-900 mt-1">{stats.averageScore}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        ...
        {results.length > 0 ? (
          <ResultTable results={results} onViewResult={handleViewCandidateResult} />
        ) : (
          <div className="py-12 text-center">
            <p className="text-sm text-gray-500">No results available yet</p>
            <p className="text-xs text-gray-400 mt-1">Candidates will appear here once they submit the test</p>
          </div>
        )}
      </div>
    </div>
